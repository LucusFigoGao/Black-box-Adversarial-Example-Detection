...
'''
-----------------------------------
    File name:   helps_train.py
    Time:        2022/09/15 21:39:15
    Editor:      Figo
-----------------------------------
'''
import os
...
from torch.optim.lr_scheduler import StepLR, MultiStepLR, CosineAnnealingLR, CosineAnnealingWarmRestarts, LambdaLR
...
def make_optimizer_and_schedule(args, model, checkpoint, params):
    ...
    *Internal Function* (called directly from train_model)
    ...
    if args.custom_lr_multiplier == 'cyclic':
        ...
        schedule = StepLR(optimizer, step_size=args.step_lr, gamma=args.step_lr_gamma)
